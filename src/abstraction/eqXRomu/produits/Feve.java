...
public enum Feve implements IProduit {
	...
	F_HQ_E( Gamme.HQ, true), // FEVE HAUTE QUALITE EQUITABLE
	F_HQ(   Gamme.HQ, false), // FEVE HAUTE QUALITE NON EQUITABLE
	F_MQ_E( Gamme.MQ, true), // FEVE MOYENNE QUALITE EQUITABLE
	F_MQ  ( Gamme.MQ, false),// FEVE MOYENNE QUALITE NON EQUITABLE
	F_BQ_E( Gamme.BQ, true), // FEVE BASSE QUALITE EQUITABLE
	F_BQ(   Gamme.BQ, false);// FEVE BASSE QUALITE NON EQUITABLE
	
	private Gamme gamme;
	private boolean equitable;

	
	Feve(Gamme gamme, boolean equitable) {
		this.gamme = gamme;
	 ...
	 */
	public static Feve get(Gamme gamme, boolean equitable) {
		for (Feve c : Feve.values()) {
			if (c.gamme==gamme && c.equitable==equitable) {
				return c;
			}
		}
		return null;
	...
	public static void main(String[] args) {
		...
		System.out.println("\nFeve.get(Gamme.MQ, ,false, true)="+Feve.get(Gamme.MQ, true));
