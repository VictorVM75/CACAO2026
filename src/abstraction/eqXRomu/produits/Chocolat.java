...
public enum Chocolat implements IProduit {
	C_HQ_E(Gamme.HQ, true), // CHOCOLAT HAUTE QUALITE  EQUITABLE
	C_HQ(  Gamme.HQ, true), // CHOCOLAT HAUTE QUALITE NON EQUITABLE
	C_MQ_E( Gamme.MQ, true), // CHOCOLAT MOYENNE QUALITE EQUITABLE
	C_MQ  ( Gamme.MQ, false),// CHOCOLAT MOYENNE QUALITE (NI BIO NI EQUITABLE)
	C_BQ_E( Gamme.BQ, true), // CHOCOLAT BASSE QUALITE EQUITABLE
	C_BQ(   Gamme.BQ, false);// CHOCOLAT BASSE QUALITE (NI BIO NI EQUITABLE)
	
	private Gamme gamme;
	private boolean equitable;
	
	Chocolat(Gamme gamme, boolean equitable) {
		this.gamme = gamme;
	...
	public static Chocolat get(Gamme gamme, boolean equitable) {
		for (Chocolat c : Chocolat.values()) {
			if (c.gamme==gamme && c.isEquitable()==equitable) {
				return c;
			}
		}
		return null;
	...
	public static void main(String[] args) {
		...
		System.out.println("\n Chocolat.get(Gamme.HQ, true, true) -> "+Chocolat.get(Gamme.HQ, true));
